...
const CheckPointStatus = () => {
  ...
  const fetchRooms = async (floorId) => {
    ...
    // const response = await axios.post(`http://localhost:5000/get-roomsCheckpoint-byfloorid`, { apiKey: AuthApiKey, one: floorId }); 
  ...
  return (
    <>
      ...
      <p style={{ color: "rgb(143 172 93)", fontSize: "22px", fontWeight: "500", marginBottom: "0px" }}>Check Point Status</p>
              ...
              <tbody>
                {rooms.map((room) => (
                  <tr key={room.pec_rm_id}>
                    ...
                    <td>{room.pec_rmde_pest_product === null ? '---' : room.pec_rmde_pest_product}</td>
                    ...
                    <td>{room.pec_rmds_desc === null ? '---' : room.pec_rmds_desc}</td>
                   
                    <td>{room.pec_rmds_sdate ? room.pec_rmds_sdate.split(' ')[0] : '---'}</td>
